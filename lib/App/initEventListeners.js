...
module.exports = function() {
	...
	this.on('layout modified', function*() {
		...
		if (this.liveReload) this.liveReload.reload()
	})
	...
	this.on('ready', () => {
		...
		console.log(`${this.config.title}`, this.chalk.dim('started on'), this.chalk.green(`${this.server.protocol}://localhost:${this.server.port}`) + '.')
		this.separator()
	})
	...
	// Prevent methods whose name starts with 'init' to be called twice
	this.on('ready', () => {
		Object.keys(Object.getPrototypeOf(this))
		.filter(method => method.startsWith('init'))
		.forEach(method => this[method] = () => null)
	})
}